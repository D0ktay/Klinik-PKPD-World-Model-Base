...
class TransientDynamicsDataset(Dataset):
    def __init__(self, npz_path: str, split: str, norm_stats: NormStats | None = None):
        ...
        for local_i, row_idx in enumerate(row_indices):
            row = {f: data[f][row_idx] for f in _PATIENT_FIELD_NAMES}
            row["comorbidity"] = str(data["comorbidity"][row_idx])
            patient_covariates[local_i] = build_patient_covariate_vector(row)

        states = np.stack([
            build_state_vector(data["traj_p"][row_idx], data["traj_v"][row_idx],
                                patient_covariates[local_i], current_hr=float(data["current_hr"][row_idx]))
            for local_i, row_idx in enumerate(row_indices)
        ])
        scalars = {
            "ef": data["ef"][row_indices], "co": data["co"][row_indices],
            "hr": data["current_hr"][row_indices],
            "edv": data["edv"][row_indices], "esv": data["esv"][row_indices],
        }
        actions_raw = data["conc_mg_L"][row_indices]
        trajectory_ids = data["trajectory_id"][row_indices]
        frame_idxs = data["frame_idx"][row_indices]
