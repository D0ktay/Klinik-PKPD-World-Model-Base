...
def load_full_length_trajectories(npz_path: str, split: str, norm_stats):
    ...
    for row_idx in row_indices:
        trajectories.setdefault(int(data["trajectory_id"][row_idx]), []).append(int(row_idx))
    ...
    for tid in full_traj_ids:
        rows = sorted(trajectories[tid], key=lambda r: int(data["frame_idx"][r]))
        traj_states = []
        for row_idx in rows:
            ...
            state = build_state_vector(data["traj_p"][row_idx], data["traj_v"][row_idx],
                                        covariates, current_hr=float(data["current_hr"][row_idx]))
            traj_states.append(norm_stats.normalize_state(state))
        traj_actions = [norm_stats.normalize_action(np.array([data["conc_mg_L"][row_idx]]))
                         for row_idx in rows]
